...
This script trains RoBERTa independently without ensemble
"""
...
try:
    from config_english import ROBERTA_CONFIG, TRAIN_CONFIG, DATA_FILES, DEVICE_CONFIG
except ImportError:
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(ROBERTA_CONFIG['log_dir'] / f'training_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
def main():
    ...
    logger.info("Starting RoBERTa training for fake news detection")
    logger.info(f"Configuration: {ROBERTA_CONFIG}")
    ...
    logger.info(f"Loading RoBERTa model: {ROBERTA_CONFIG['model_name']}")
    ...
    optimizer = AdamW(
        model.parameters(),
        lr=ROBERTA_CONFIG['learning_rate'],
    ...
    for epoch in range(ROBERTA_CONFIG['epochs']):
        ...
        train_metrics = train_epoch(
            model, train_loader, optimizer, scheduler, device, scaler, ROBERTA_CONFIG
        )
        ...
        if val_metrics['f1'] > best_val_f1:
            ...
            save_checkpoint(model, tokenizer, optimizer, epoch, val_metrics, ROBERTA_CONFIG)
